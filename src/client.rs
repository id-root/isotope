    ...
    loop {
        ...
        if let Ok(data) = bincode::serialize(&join_msg) {
             if let Ok(pkt) = IsotopePacket::new(&data) {
                 let enc_res = session.lock().unwrap().encrypt(&pkt.to_bytes().unwrap());
        ...
        let mut last_typing_sent = std::time::Instant::now() - Duration::from_secs(10); // Initialize as long ago
        ...
        'session: loop {
            ...
            // B10: Update Dashboard Metrics (Single values only)
                        ...
                        match key.code {
                            ...
                            KeyCode::Enter if app.focus == Focus::Input => {
                                if key.modifiers.contains(KeyModifiers::SHIFT) || key.modifiers.contains(KeyModifiers::ALT) {
                                    ...
                                    // B7: Check for /search command
                                    if input.starts_with("/search ") {
                                        let query = input.trim_start_matches("/search ").to_string();
                                        app.perform_search(query);
                                        app.focus = Focus::Chat; // Move focus to chat to navigate results
                                    } else if input.trim() == "/search" {
                                        // Just clear/reset search
                            ...
                            KeyCode::Esc => { 
                                if app.is_searching {
                                    app.perform_search("".to_string()); // Clear search
                        ...
                        match msg {
                            ...
                            WireMessage::DirectMessage { sender, content, ttl, .. } => {
                                if let Some(seconds) = ttl {
                                    ...
                                    // B3: Send Read Receipt
                                    // We don't have msg_id easily here from add_msg default return, 
                                    // but we can assume the last added message is the one.
                                    // Actually add_msg returns id.
                                    // let msg_id = ... wait add_msg is void? No it returns u64 in my memory.
                                    // checking app.rs... Yes it returns u64.
                                }
                            },
                            ...
                            WireMessage::FileOffer { sender, file_name, file_size, id, .. } => {
                                if file_size > MAX_FILE_SIZE {
                                    ...
                                    // Show file offer in main chat so user sees it immediately
                            ...
                            WireMessage::FileRequest { file_id, receiver } => {
                                if let Some(path) = upload_queue.get(&file_id) {
                                    // Show upload start in main chat
                                    ...
                                    if let Some((name, progress)) = active_downloads.get_mut(&file_id) {
                                        ...
                                        *progress = chunk_index;
                                        ...
                                        if chunk_index == total_chunks - 1 {
                                            // Show download complete in main chat
