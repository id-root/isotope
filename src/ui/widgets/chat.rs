...
pub fn draw(f: &mut Frame, app: &mut AppState, area: Rect) {
    // B2: Cleanup stale typing indicators
    app.cleanup_typing();
    ...
    let chunks = Layout::default()
        ...
        .constraints([
            ...
            Constraint::Length(1), // B2: Typing indicator
    ...
    let messages: Vec<ListItem> = app.messages.iter().enumerate().rev().map(|(i, msg)| { // enumerate for search index
        let sender = &msg.sender;
        let content = &msg.content;
        let time_str = msg.timestamp.format("%H:%M").to_string();

        // B7: Search Highlighting
        let is_match = app.search_matches.contains(&i);
        let is_current_match = app.current_match_index.map(|idx| app.search_matches.get(idx).map(|&m| m == i).unwrap_or(false)).unwrap_or(false);
        ...
        // Apply search highlight to header if matched
        ...
        let header_span = Span::styled(format!("[{}] {}: ", time_str, prefix), header_style);
        ...
        // B3: Add read receipt indicator
        if !msg.read_by.is_empty() && msg.sender == app.username {
            let check = if msg.read_by.len() > 0 { "✓✓" } else { "✓" };
            lines.push(Line::from(vec![
                Span::raw(" ".repeat(time_str.len() + 4)),
    ...
    let typing_text = if typing_users.is_empty() {
        String::new()
    ...
    let typing_para = Paragraph::new(typing_text)
        .style(Style::default().fg(Color::DarkGray).add_modifier(Modifier::ITALIC));
    f.render_widget(typing_para, chunks[1]);
