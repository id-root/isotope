...
pub enum WireMessage {
    // [FIX] V12: Protocol Versioning
    Version { major: u8, minor: u8 },
    Signal(SignalType),
    Heartbeat,
    // T1: Cover Traffic - dummy packets to mask real traffic
    Dummy { noise: Vec<u8> },
    ...
    // C2: PQ Key Rotation - request/response for rekey
    PQRekey {
        public_key: Vec<u8>,
    },
    PQRekeyFinish {
...
impl IsotopePacket {
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>> {
        ...
        if data.len() < PLAINTEXT_SIZE {
            ...
            // FIX #5: Error instead of silent truncation
            bail!("Serialized packet exceeds PLAINTEXT_SIZE limit ({} > {})", data.len(), PLAINTEXT_SIZE);
        }
        Ok(data)
    }
