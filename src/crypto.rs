...
impl Identity {
    ...
    pub fn load<P: AsRef<Path>>(path: P, password: &str) -> Result<Self> {
        ...
        // FIX #1: Derive separate nonces per slot using slot index as domain separator
        let nonce_slot1 = Self::derive_nonce(password, salt, 0);
        let nonce_slot2 = Self::derive_nonce(password, salt, 1);
    ...
    pub fn setup_dual<P: AsRef<Path>>(path: P, pass_ops: &str, pass_casual: &str) -> Result<()> {
        ...
        // Encrypt Ops -> Slot 1 (slot_index = 0)
        ...
        // FIX #1: Use slot-specific nonce derivation
        let nonce_ops = Self::derive_nonce(pass_ops, &salt, 0);
        ...
        // FIX #1: Use slot-specific nonce derivation (slot_index = 1)
        let nonce_casual = Self::derive_nonce(pass_casual, &salt, 1);
    ...
    /// FIX #1: Derive nonce with slot index for domain separation
    /// This prevents nonce reuse if different passwords happen to collide
    fn derive_nonce(password: &str, salt: &[u8], slot_index: u8) -> Nonce {
        let mut nonce_material = [0u8; 12];
        let mut h = Hasher::new();
        h.update(password.as_bytes());
        h.update(b"NONCE_DOMAIN");
        h.update(salt);
        h.update(&[slot_index]); // Domain separator per slot
...
pub struct NoiseSession {
    ...
    // C2: PQ Key Rotation - track message count (FIX #11: Infrastructure for future integration)
    #[allow(dead_code)]
...
// C2: Rekey thresholds - FIX #11: Infrastructure for future integration
#[allow(dead_code)]
...
impl NoiseSession {
    ...
    // C2: Check if session needs PQ key rotation - FIX #11: Infrastructure for future integration
    #[allow(dead_code)]
    ...
    // C2: Increment message counter - FIX #11: Infrastructure for future integration
    #[allow(dead_code)]
    pub fn increment_message_count(&mut self) {
        self.message_count += 1;
    }
    
    // FIX #11: Reset rekey state after successful key rotation
    #[allow(dead_code)]
